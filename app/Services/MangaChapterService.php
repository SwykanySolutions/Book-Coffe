...
class MangaChapterService
{
    ...
    public function createChapterManga(Request $request)
    {
        ...
        foreach ($request->file('pages') as $pages) {
            ...
            $width = $image_info[0];
            $height = $image_info[1];
            $uploadedPages[] = ['page' => $pages->store('chapters/pages/' . $chapter->id), 'chapter_manga_id' => $chapter->id, 'created_at' => date('Y-m-d h:i:s'), 'updated_at' => date('Y-m-d h:i:s'), 'width' => $width, 'height' => $height];
        }
